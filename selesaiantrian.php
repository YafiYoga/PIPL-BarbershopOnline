...
<body>
    ...
    <table>
        ...
        <tbody>
            <?php
            $no = 0;
            $ambildata = mysqli_query($connection, "SELECT * FROM user INNER JOIN antrian ON user.user_id = antrian.id_customer WHERE antrian.id_barber = $_SESSION[user_id] AND antrian.status = 'selesai'");
